...
- **D-02:** Remove the incompatible `roave/security-advisories` metapackage only when it is replaced by Composer's native dependency-security policy and a `composer audit --locked` gate. Do not use platform emulation, platform-ignore flags, policy-disabling flags, package-wide ignores, severity-wide ignores, or a non-blocking audit mode. **User-approved, internal-only exception (2026-07-22):** permit an `ignore-id` exception only for the three currently applicable `laravel/framework` advisory IDs — `PKSA-m5cs-t1y6-qpcs`, `PKSA-3r5d-mb8f-1qw9`, and `PKSA-mdq4-51ck-6kdq` — each with the documented reason that the application is internal-only and the project owner accepted the residual risk. The exception expires at Phase 2 lockfile review or when a compatible stable SendPortal Core release allows an upgraded Laravel line, whichever is first; all other advisories must remain blocking and failing.
