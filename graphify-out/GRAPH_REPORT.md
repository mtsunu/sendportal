...
## Corpus Check
- 257 files · ~503,322 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2000 nodes · 2488 edges · 197 communities (161 shown, 36 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 59 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `fdaecd84`
...
- Security Domain
- Code Examples
- SesThrottleDetectionTest
- Sources
- AcceptInvitation
- self
- ResetPasswordController
- ThrottledSesAdapter.php
- Illuminate\Foundation\Testing\RefreshDatabase
- RuntimeException
- ApiTokenController
- ExistingUserInvitationTest
- SesExhaustionTest
- ResetPasswordController
- ComposerPolicyCommandContract
- HorizonServiceProvider
...
1. `Communities (196 total, 35 thin omitted)` - 126 edges
2. `User` - 47 edges
3. `Workspace` - 37 edges
4. `Controller` - 32 edges
5. `TestCase` - 30 edges
6. `ThrottledSesAdapter` - 26 edges
7. `auditRoutes()` - 26 edges
8. `Invitation` - 23 edges
9. `InstallApplication` - 22 edges
10. `Phase 2: Reproducible Dependency Snapshot - Research` - 21 edges
...
- `throttledAdapter()` --references--> `ThrottledSesAdapter`  [EXTRACTED]
  tests/Feature/Ses/SesAdapterTestSupport.php → app/Mail/ThrottledSesAdapter.php
...
Cohesion: 0.21
Nodes (15): Workspace, currentWorkspace(), currentWorkspaceId(), getCurrentWorkspaceAttribute(), hasWorkspaces(), invitations(), onWorkspace(), ownsCurrentWorkspace() (+7 more)

### Community 1 - "ComposerPolicyGuardTest.php"
Cohesion: 0.10
Nodes (57): assertFixtureRouteFails(), assertFixtureRouteHasNoMutation(), assertGuardStructure(), assertionEnvironment(), assertNoComposerRan(), assertOnlyVersionProbeRan(), assertParserRejects(), assertTrue() (+49 more)
...
### Community 4 - "Setup"
Cohesion: 0.06
Nodes (10): Setup, Database, Env, Key, Migrations, Url, Illuminate\Validation\Rule, Livewire\Component (+2 more)
...
Cohesion: 0.14
Nodes (13): Anti-patterns to avoid, AppServiceProvider static-map rebind — soundness, Architecture Research, Component responsibilities (target design), Limiter key and MaxSendRate cache placement, Making BLOCK safe against Horizon timeouts and `tries=3`, Recommended build order (exact host files), Recommended `ThrottledSesAdapter::send()` shape (+5 more)

### Community 10 - "Illuminate\Contracts\View\View"
Cohesion: 0.14
Nodes (7): LoginController, ProfileController, Workspace, WorkspacesController, ProfileUpdateRequest, Illuminate\Contracts\View\View, Illuminate\Foundation\Auth\AuthenticatesUsers

### Community 11 - "Illuminate\Foundation\Testing\RefreshDatabase"
Cohesion: 0.06
Nodes (30): Automated (per task), - Aws\Result is array-constructible: new Aws\Result(['MessageId' => 'id']) / new Aws\Result(['MaxSendRate' => 14.0])., - Aws\Ses\Exception\SesException: getAwsErrorCode() reads $context['code'] (the outer detection, Chain: ->allow(int $rate)->every(1)->block(int $seconds)->sleep(int $ms)->then(callable $ok, ?callable $onFail)., Code this plan extends / mocks (read once, extract signatures), $context['message']); the production sub-branch matches on getAwsErrorMessage(). Construct in, do NOT match on it. The clean AWS wording lives in getAwsErrorMessage() (reads, gate: === 'Throttling'). getMessage() is a VERBOSE wrapper ("Error executing SendEmail…") — (+22 more)

### Community 12 - "Closure"
Cohesion: 0.17
Nodes (5): LocaleMiddleware, OwnsCurrentWorkspace, RedirectIfAuthenticated, RequireWorkspace, Closure

### Community 13 - "Controller"
Cohesion: 0.15
Nodes (10): ForgotPasswordController, VerificationController, Controller, SetupController, Illuminate\Foundation\Auth\Access\AuthorizesRequests, Illuminate\Foundation\Auth\SendsPasswordResetEmails, Illuminate\Foundation\Auth\VerifiesEmails, Illuminate\Foundation\Bus\DispatchesJobs (+2 more)
...
Cohesion: 0.21
Nodes (6): PendingInvitationController, SwitchWorkspaceController, WorkspaceUsersController, OwnsRequestedWorkspace, Illuminate\Http\RedirectResponse, Illuminate\Http\Request
...
Cohesion: 0.10
Nodes (20): Context anchors (verified from code), Critical Pitfalls, Integration Gotchas, "Looks Done But Isn't" Checklist, Performance Traps, Pitfall 1: Bug-1 "fix" turns a precise check into a false-positive — daily-quota-exceeded gets retried as if it were a rate limit, Pitfall 2: Double-send — SES accepts the email but `sent_at` is never persisted, so `tries=3` re-sends it, Pitfall 3: Limiter is not actually shared across processes (in-memory / wrong cache store or connection) (+12 more)

### Community 21 - "Illuminate\Foundation\Http\FormRequest"
Cohesion: 0.16
Nodes (5): ApiTokenStoreRequest, CreateWorkspaceRequest, WorkspaceStoreRequest, WorkspaceUpdateRequest, Illuminate\Foundation\Http\FormRequest
...
Cohesion: 0.12
Nodes (15): Alternatives Considered, Caveats the planner must carry forward, Core Technologies, Development Tools, Installation, Recommended Stack, Redis client / config that Horizon implies is available, Scope note (+7 more)

### Community 25 - "TestCase"
Cohesion: 0.02
Nodes (126): Communities (196 total, 35 thin omitted), Community 0 - "Workspace", Community 103 - "01-01-PLAN.md", Community 104 - "01-09-PLAN.md", Community 105 - "01-10-PLAN.md", Community 106 - "01-11-PLAN.md", Community 107 - "Standard Stack", Community 108 - "User Constraints (from CONTEXT.md)" (+118 more)
...
Cohesion: 0.10
Nodes (19): Add After Validation (v1.x), Anti-Features (avoid — scope creep or incorrect), AWS Technical Reference (authoritative — planner must build against these), Dependency Notes, Differentiators / Nice-to-have (defer unless cheap), Feature Dependencies, Feature Landscape, Feature Prioritization Matrix (+11 more)

### Community 35 - "self"
Cohesion: 0.14
Nodes (8): RegisterController, ValidInvitation, CreateWorkspace, getInvitationFromToken(), isInvalidInvitation(), isValidInvitation(), Illuminate\Contracts\Validation\Rule, Illuminate\Foundation\Auth\RegistersUsers
...
Cohesion: 0.14
Nodes (13): Active, Constraints, Context, Core Value, Current Milestone: v1.2 Saved Sender Identities, Current State, Evolution, Key Decisions (+5 more)
...
Cohesion: 0.11
Nodes (9): Invitation, Workspace, SendInvitation, Carbon, Carbon\Carbon, Illuminate\Database\Eloquent\Factories\HasFactory, Illuminate\Database\Eloquent\Model, Illuminate\Database\Eloquent\Relations\BelongsTo (+1 more)
...
Cohesion: 0.10
Nodes (19): Cost Observations, Cost Observations, Cross-Milestone Trends, Cumulative Quality, Key Lessons, Key Lessons, Milestone: v1.0 — PHP 8.4 Compatibility, Milestone: v1.1 — SES Sending Reliability (+11 more)
...
Cohesion: 0.20
Nodes (9): Milestones, Phase 5: Sender Identity Management, Phase 6: Campaign Form Sender Selection, Phase 7: Sender Auto-Capture & Data Integrity, Phase Details, Phases, Progress, Roadmap: SendPortal PHP 8.4 Compatibility (+1 more)
...
Cohesion: 0.17
Nodes (11): Composer Compatibility, Continuous Integration, Dependency Security & Reproducibility, Out of Scope, Requirements Archive: v1.0 PHP 8.4 Compatibility, Requirements: SendPortal PHP 8.4 Compatibility, Runtime Validation, Traceability (+3 more)
...
Cohesion: 0.15
Nodes (7): WorkspacesRepository, AcceptInvitation, Workspace, AddWorkspaceMember, Workspace, Illuminate\Contracts\Pagination\LengthAwarePaginator, Sendportal\Base\Repositories\BaseEloquentRepository

### Community 83 - "VerificationController"
Cohesion: 0.17
Nodes (11): Anti-Patterns Found, Deviation Assessment, Gaps Summary, Goal Achievement, Key Link Verification, Minor observation (non-blocking, WARNING), Observable Truths / Success Criteria, Outstanding CI Gates (must pass before ship) (+3 more)
...
Cohesion: 0.20
Nodes (9): Deferred (not this phase), Goal restated, Key pitfalls (with prevention → owning build step), Locked technical decisions (design contract — do not re-litigate), New / changed host files (build order — internal to this phase), Phase 4 Research: Coordinated SES rate limiting + 2 bug fixes, Sources, Validation Architecture (+1 more)

### Community 87 - "HorizonServiceProvider"
Cohesion: 0.20
Nodes (9): Campaign Integration, Delivery Integrity, Future Requirements, Milestone v1.2 Requirements, Out of Scope, Requirements: SendPortal — v1.2 Saved Sender Identities, Sender Enhancements, Sender Management (+1 more)
...
Cohesion: 0.09
Nodes (14): User, RemoveUserFromWorkspace, Admin, Illuminate\Contracts\Auth\MustVerifyEmail, Illuminate\Foundation\Auth\User, Illuminate\Notifications\Notifiable, Illuminate\Testing\TestResponse, WorkspaceUserControllerTest (+6 more)
...
Cohesion: 0.20
Nodes (9): Future Requirements, Out of Scope, Quality / CI (separate future milestone), Requirements Archive: v1.1 SES Sending Reliability, Requirements: SendPortal — v1.1 SES Sending Reliability, SES Sending Rate, SES Sending Rate, Traceability (+1 more)
...
### Community 115 - "WritesToEnvironment.php"
Cohesion: 0.22
Nodes (5): AppServiceProvider, BroadcastServiceProvider, Illuminate\Support\ServiceProvider, auth.partials.logo, setup

### Community 116 - "autoload-dev"
Cohesion: 0.67
Nodes (3): autoload-dev, psr-4, Tests\\

### Community 117 - "01-12-PLAN.md"
Cohesion: 0.40
Nodes (4): Artifacts this phase produces, Source Coverage Audit, STRIDE Threat Register (ASVS L1), Trust Boundaries

### Community 118 - "02-01-PLAN.md"
Cohesion: 0.40
Nodes (4): Artifacts this phase produces, Source Coverage Audit, STRIDE Threat Register (ASVS L1), Trust Boundaries

### Community 119 - "02-02-PLAN.md"
Cohesion: 0.40
...
### Community 134 - "LoginController"
Cohesion: 0.18
Nodes (5): Illuminate\Foundation\Testing\TestCase, SesAdapterTestSupport, SesRateLimitCrossProcessTest, SesTestCase, ThrottledSesAdapterTracerTest

### Community 138 - "SesAdapterTestSupport.php"
Cohesion: 0.24
Nodes (12): Aws\Ses\Exception\SesException, Aws\Ses\SesClient, Mockery\MockInterface, Sendportal\Base\Models\EmailService, dailyQuotaException(), rateExceededException(), sesClient(), sesClientMock() (+4 more)

### Community 170 - "01-09-PLAN.md"
Cohesion: 0.50
Nodes (3): Source Coverage Audit, STRIDE Threat Register, Trust Boundaries

### Community 171 - "01-10-PLAN.md"
Cohesion: 0.50
Nodes (3): Source Coverage Audit, STRIDE Threat Register, Trust Boundaries
...
Cohesion: 0.50
Nodes (3): Milestones, v1.0 PHP 8.4 Compatibility (Shipped: 2026-07-25), v1.1 SES Sending Reliability (Shipped: 2026-07-25)

### Community 179 - "Security Domain"
Cohesion: 0.67
...
### Community 184 - "self"
Cohesion: 0.21
Nodes (3): self, SesDoubleSendTest, SesRateSourceTest

### Community 185 - "ResetPasswordController"
Cohesion: 0.22
Nodes (8): Manual-Only Verifications, Per-Task Verification Map, Phase 4 — Validation Strategy, Sampling Rate, status lifecycle: draft (seeded by plan-phase) → validated (set by validate-phase §6), Test Infrastructure, Validation Sign-Off, Wave 0 Requirements

### Community 186 - "ThrottledSesAdapter.php"
Cohesion: 0.25
Nodes (7): Milestones, Phase 4: Coordinated SES rate limiting + 2 bug fixes, Phase Details, Phases, Progress, Roadmap: SendPortal PHP 8.4 Compatibility, 🚧 v1.1 SES Sending Reliability (Phase 4)

### Community 187 - "Illuminate\Foundation\Testing\RefreshDatabase"
Cohesion: 0.07
Nodes (13): CreatesApplication, Illuminate\Foundation\Testing\RefreshDatabase, Illuminate\Foundation\Testing\WithFaker, AuthConfigDisabledTest, AuthConfigEnabledTest, ExistingUserInvitationTest, NewUserInvitationTest, SetupControllerTest (+5 more)

### Community 188 - "RuntimeException"
Cohesion: 0.17
Nodes (6): SesSendThrottledException, checkEnvValuePresent(), writeToEnvironmentFile(), RuntimeException, Sendportal\Base\Adapters\SesMailAdapter, Sendportal\Base\Services\Messages\MessageTrackingOptions

### Community 189 - "ApiTokenController"
Cohesion: 0.33
Nodes (3): ApiTokenController, ApiTokenRepository, Sendportal\Base\Repositories\BaseTenantRepository

### Community 190 - "ExistingUserInvitationTest"
Cohesion: 0.27
Nodes (3): WorkspaceInvitationsController, WorkspaceInvitationStoreRequest, Illuminate\Contracts\Validation\Validator

### Community 196 - "Phase 4 Context: Coordinated SES rate limiting + 2 bug fixes"
Cohesion: 0.40
Nodes (4): Claude's discretion (planner may decide), Key references, Locked decisions, Phase 4 Context: Coordinated SES rate limiting + 2 bug fixes

## Knowledge Gaps
- **1002 isolated node(s):** `name`, `type`, `description`, `license`, `php` (+997 more)
...
- **Why does `ComposerPolicyCommandContract` connect `ComposerPolicyCommandContract` to `ComposerPolicyGuardTest.php`?**
  _High betweenness centrality (0.021) - this node is a cross-community bridge._
- **Why does `User` connect `WorkspaceInvitationsControllerTest` to `Workspace`, `self`, `InstallApplication`, `ApiTokenController`, `Controller`, `SetupTest`, `Illuminate\Http\RedirectResponse`, `Common Pitfalls`, `Illuminate\Foundation\Testing\RefreshDatabase`?**
  _High betweenness centrality (0.019) - this node is a cross-community bridge._
- **Why does `parseInvocation()` connect `ComposerPolicyGuardTest.php` to `ComposerPolicyCommandContract`?**
...
- **Are the 38 inferred relationships involving `self` (e.g. with `.resolveWorkspaceId()` and `.a_user_can_accept_valid_invitations()`) actually correct?**
  _`self` has 38 INFERRED edges - model-reasoned connections that need verification._
