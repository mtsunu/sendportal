...
**`$gsd-next`** — State-aware front door. Detects your situation via `node "$HOME/.codex/gsd-core/bin/gsd-tools.cjs" smart-entry` (no-project, paused, blocked, planning, executing, needs-verify, idle, complete, …) and shows a menu with one recommended action. Launcher only; falls back to `$gsd-progress`.
...
**`$gsd-spike --wrap-up`** — Curates spikes one-at-a-time (include/exclude/partial/UAT), generates a project skill under `./.codex/skills/spike-findings-[project]/`, writes `.planning/spikes/WRAP-UP-SUMMARY.md`, adds a AGENTS.md auto-load line.

Usage: `$gsd-spike --wrap-up`

**`$gsd-sketch --wrap-up`** — Same curation flow for sketches, generating `./.codex/skills/sketch-findings-[project]/` with design decisions/CSS/HTML structures.
...
**`$gsd-capture --note <text>`** — Zero-friction timestamped note to `.planning/notes/` (or `/Users/meigire/Work/idai-jatim/sendportal/.codex/notes/` globally). Subcommands: append (default), list, promote (note → todo). Works without a project.
