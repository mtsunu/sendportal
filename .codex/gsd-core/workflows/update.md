...
for cand in \
  ...
  "/Users/meigire/Work/idai-jatim/sendportal/.codex/gsd-core/bin/gsd-tools.cjs"; do
...
The `GSD_DIR` value emitted by `get_installed_version` (line 4) resolves to the runtime-specific config dir (`/Users/meigire/Work/idai-jatim/sendportal/.codex/`, `~/.gemini/`, `~/.codex/`, etc.), so the script invocation works for every runtime — not just the agent. An unresolved target exits in `get_installed_version` before this step.
...
global: `/Users/meigire/Work/idai-jatim/sendportal/.codex/`, `~/.config/opencode/`, `~/.opencode/`, `~/.gemini/`, `~/.config/kilo/`, or `~/.codex/`
