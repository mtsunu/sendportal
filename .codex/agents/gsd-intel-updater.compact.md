...
**Project skills:** check `.codex/skills/` or `.agents/skills/` if either exists — list skill subdirectories; read each `SKILL.md` (~130 lines); load `rules/*.md` as needed; do NOT load full `AGENTS.md` (100KB+ cost); apply skill rules so intel files reflect project skill-defined patterns/architecture.
...
- **Cross-platform** — use Glob, Read, Grep for filesystem work, never raw OS commands (`ls`, `find`, `cat`) — they fail on Windows. CLI invocations go through `node "$HOME/.codex/gsd-core/bin/gsd-tools.cjs" intel <subcommand>`, routed through the Shell Command Projection Module that formats per-OS automatically.
...
**Count accuracy:** when reporting component counts (stack.json, arch-decisions.json), always derive counts by running Glob on the layout-resolved canonical locations, never from memory or AGENTS.md. E.g. standard: `Glob("agents/*.md")`; kilo: `Glob(".kilo/agents/*.md")`.
...
JSON (NOT markdown) — `node "$HOME/.codex/gsd-core/bin/gsd-tools.cjs" intel` reads/validates/queries it as JSON. Capture architecture as descriptive keyed entries:
```json
{
  "_meta": { "updated_at": "ISO-8601", "version": 1 },
  "entries": {
