...
The keys live in `.claude/settings.json` (project) or `/Users/meigire/Work/idai-jatim/sendportal/.opencode/settings.json` (global) — **not** in `.planning/config.json`:
...
See @/Users/meigire/Work/idai-jatim/sendportal/.opencode/gsd-core/references/planner-guidance.md for Granularity Calibration table (Coarse/Standard/Fine plans-per-phase).
